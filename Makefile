...
PROJECT_NAME=intranet-trems
STACK_NAME=intranet-tre-ms-jus-br
...
stack-start:  ## Local Stack: Start Services
	...
	@echo "Now visit: http://intranet-trems.localhost"
...
build-acceptance-servers: ## Build Acceptance Servers
	...
	@docker build backend --build-arg PLONE_VERSION=${PLONE_VERSION} -t William-Ourives/intranet-trems-backend:acceptance -f backend/Dockerfile.acceptance
	@echo "Build acceptance frontend"
	@docker build frontend --build-arg VOLTO_VERSION=${VOLTO_VERSION} -t William-Ourives/intranet-trems-frontend:acceptance -f frontend/Dockerfile

.PHONY: start-acceptance-servers
start-acceptance-servers: build-acceptance-servers ## Start Acceptance Servers
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name intranet-trems-backend-acceptance -d William-Ourives/intranet-trems-backend:acceptance
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name intranet-trems-frontend-acceptance --link intranet-trems-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d William-Ourives/intranet-trems-frontend:acceptance

.PHONY: stop-acceptance-servers
stop-acceptance-servers: ## Stop Acceptance Servers
	@echo "Stop acceptance containers"
	@docker stop intranet-trems-frontend-acceptance
	@docker stop intranet-trems-backend-acceptance
